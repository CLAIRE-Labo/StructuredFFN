...
def main(config):
    ...
    model.eval()
    f = open("../../exp/logs/arch_train_latency.log", "a+")
    print(
        "h-f-a-nq-nkv",
    ...
    with torch.no_grad():
        input_ids = torch.zeros(config.data.test.test_batch, 1024).long().cuda()
        model.to(torch_dtype)
        latency, throughput = benchmark_train(model, input_ids)
    params = sum(p.numel() for p in model.parameters())
    params_woemb = params - 32000 * config.model.kwargs.hidden_dim
    print(
        f"bs: {config.data.test.test_batch}, latency: {latency}, params: {params}, params_woemb: {params_woemb}, throughput: {throughput}",
        file=f,
    )
